...
export const WorkflowEditor: React.FC<WorkflowEditorProps> = ({
  ...
  useEffect(() => {
    ...
    console.log('Project config extensions:', projectConfig.extensions);
    console.log('Full project config:', projectConfig);
    console.log('Project data:', projectConfig.projectData);
    console.log('Project data type:', projectConfig.projectData?.type);
    console.log('Project data project_type:', projectConfig.projectData?.project_type);
    
    // Check if this should be a DAL project
    const shouldBeDALProject = checkIfShouldBeDALProject(projectConfig.projectData);
    console.log('Should be DAL project:', shouldBeDALProject);
    
    if (projectConfig.extensions?.dal) {
      console.log('Found DAL extension:', projectConfig.extensions.dal);
      setAlConfig(projectConfig.extensions.dal as ALConfiguration);
    } else if (shouldBeDALProject) {
      // Auto-create DAL extension for projects that should be DAL projects
      console.log('Auto-creating DAL extension for detected DAL project');
      ...
      // Update the project configuration with DAL extension
      if (account) {
        try {
          ...
          console.log('Created DAL extension for project:', projectId);
        } catch (error) {
      ...
      // Initialize default AL config for any project that might become a DAL project
      console.log('No DAL extension found, initializing default AL config');
      const defaultAlConfig: ALConfiguration = {
        queryStrategy: 'uncertainty_sampling',
        AL_scenario: 'single_annotator',
        model: {
  ...
  }, [projectConfig.extensions, projectId, account, checkIfShouldBeDALProject]);

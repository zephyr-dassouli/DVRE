...
export class DALProjectSession extends EventEmitter {
  ...
  private async loadQuerySamplesFile(filePath: string): Promise<QuerySample[]> {
    try {
      // Get the real iris samples that were generated by AL-Engine and stored by ALContractService
      console.log('📊 Loading real iris samples from ALContractService...');
      
      const storedSamples = alContractService.getStoredALSamples(this.state.projectId);
      
      if (storedSamples && storedSamples.length > 0) {
        console.log(`✅ Found ${storedSamples.length} real iris samples from AL-Engine`);
        console.log('🌸 First sample:', storedSamples[0]);
      ...
      console.warn('⚠️ No real samples found, falling back to mock data');
      
      // Fallback to mock data if no real samples available

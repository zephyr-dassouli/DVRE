    ...
    def setup_routes(self):
        ...
        def get_iteration_results(iteration):
            ...
            try:
                ...
                model_file = self.work_dir / f"model_iter_{iteration}.pkl"
                query_file = self.work_dir / f"query_indices_iter_{iteration}.npy"
                performance_file = self.work_dir / f"performance_iter_{iteration}.json"
                
                results = {
                    'iteration': iteration,
                    'files': {
                        'model': str(model_file) if model_file.exists() else None,
                        'query_indices': str(query_file) if query_file.exists() else None,
                        'performance': str(performance_file) if performance_file.exists() else None
                    }
                }

...
export class SmartContractIntegration {
     ...
     */
    async updateProjectWithIPFS(data) {
        try {
            ...
            const contract = new ethers.Contract(data.contractAddress, contractInterface, this.signer);
            ...
            const transaction = await contract.updateProjectMetadata(data.ipfsRoCrateHash, data.ipfsWorkflowHash, data.ipfsBundleHash, statusCode);
     ...
     */
    async mockUpdateProjectWithIPFS(data) {
     ...
     */
    async updateProjectStatus(contractAddress, status) {
        try {
            if (!this.provider || !this.signer) {
                await this.initializeProvider();
                if (!this.provider || !this.signer) {
            ...
            const contract = new ethers.Contract(contractAddress, contractInterface, this.signer);
            const statusMap = {
                'configured': 1,
                'ready': 2,
                'active': 3,
     ...
     */
    async getProjectMetadata(contractAddress) {
        try {
            if (!this.provider) {
                await this.initializeProvider();
                if (!this.provider) {
     ...
     */
    async checkUpdatePermission(contractAddress, userAddress) {
        try {
            if (!this.provider) {
                await this.initializeProvider();
                if (!this.provider) {
    ...
    async estimateUpdateGas(data) {
        try {
            ...
            const statusCode = statusMap[data.status] || 0;
            const gasEstimate = await contract.updateProjectMetadata.estimateGas(data.ipfsRoCrateHash, data.ipfsWorkflowHash, data.ipfsBundleHash, statusCode);
...
//# sourceMappingURL=SmartContractIntegration.js.map